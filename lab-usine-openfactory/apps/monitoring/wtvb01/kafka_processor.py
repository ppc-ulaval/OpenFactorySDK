...
class KafkaProcessor:
    ...
    def safe_deserialize_key(self, x):
        ...
        try:
            ...
            return (
                "".join(c for c in clean_key if c.isprintable())
                if clean_key
                else "unknown"
            )
        except json.JSONDecodeError:
    ...
    def send_spectrogram_data(self, spectrogram_data):
        ...
        try:
            ...
            self.producer.send(self.output_topic, key="WTVB01", value=message)
            self.producer.send(self.output_topic, key="WTVB01", value=spectrogram_data)
            self.producer.flush()
    ...
    def process_message(self, key, value):
        ...
        try:
            ...
            result = self.signal_processor.compute_spectrogram(
                value["VALUE_LIST"], value["TIMESTAMPS"]
            )
